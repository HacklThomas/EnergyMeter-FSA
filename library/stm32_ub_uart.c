...
// Name, Clock               , AF-UART      ,UART  , Baud , Interrupt
				{ COM1, RCC_APB1Periph_USART3, GPIO_AF_USART3, USART3, 9600,
						USART3_IRQn, // UART2 mit 115200 Baud
// PORT , PIN      , Clock              , Source
						{ GPIOC, GPIO_Pin_10, RCC_AHB1Periph_GPIOC,
								GPIO_PinSource10 }, // TX an PA2
						{ GPIOC, GPIO_Pin_11, RCC_AHB1Periph_GPIOC,
								GPIO_PinSource11 } }, // RX an PA3
		};

//--------------------------------------------------------------
// init aller UARTs
//--------------------------------------------------------------
void UB_Uart_Init(int secondinit) {
...
//--------------------------------------------------------------
void Uart_SendString(UART_NAME_t uart, char *ptr, UART_LASTBYTE_t end_cmd) {
	// sende kompletten String
	while (*ptr != 0) {
		Uart_SendByte(uart, *ptr);
		ptr++;
	}
	// eventuell Endekennung senden
	if (end_cmd == LFCR) {
		Uart_SendByte(uart, 0x0A); // LineFeed senden
		Uart_SendByte(uart, 0x0D); // CariageReturn senden
...
//--------------------------------------------------------------
void Uart_SendArray(UART_NAME_t uart, uint8_t *data, uint16_t cnt) {
	uint32_t n;
