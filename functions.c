...
void init() {
	...
	Uart_SendString(COM1, "AT+NAMELOGGER", NONE);
...
void saveData() {
	...
	f_puts(text, &fil);
	// Versorgungsspannung
	double lowvoltage = (TM_ADC_Read(ADC1, ADC_Channel_10) / 4096.0) * 3 * 48;
	snprintf(text, sizeof(text), "%3.2f\t", lowvoltage);
	f_puts(text, &fil);
	// Spannung am Supercap
	double vbat = (TM_ADC_ReadVbat(ADC1) / 4096.0) * 3;
...
void insertHeaderToFile() {
	...
	f_puts(buffer, &fil);
	f_puts("\n", &fil);
	// Einfügen der Unique ID des Mikrocontrollers
